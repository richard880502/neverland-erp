...
async function writeMovementsToGoogleSheet(spreadsheetId: string, timeZone: string, entries: QueueMovement[]) {
  ...
  if (nonEmptyHeaders.length && outboundHeaders.some((header, index) => currentHeaders[index] !== header)) {
    throw new Error("「庫存異動」N:R 已有其他欄位，無法安全建立 ERP 同步欄位");
  }
  if (!nonEmptyHeaders.length) {
    await sheets.spreadsheets.batchUpdate({
      ...
      requestBody: {
        requests: [{
          copyPaste: {
            source: { sheetId: target.sheetId, startRowIndex: 0, endRowIndex: 1, startColumnIndex: 12, endColumnIndex: 13 },
            destination: { sheetId: target.sheetId, startRowIndex: 0, endRowIndex: 1, startColumnIndex: 13, endColumnIndex: 18 },
    ...
    await sheets.spreadsheets.values.update({
      ...
      requestBody: { values: [outboundHeaders] },
    });
  }
  ...
  for (const entry of pending) {
    if (entry.movement.type === "ADJUSTMENT") throw new Error(`異動 ${entry.movementId} 是庫存調整，原試算表尚未支援此事件`);
  }
      ...
      requestBody: {
        requests: [{
          appendDimension: {
            ...
            dimension: "ROWS",
            length: endRow - (target.gridProperties?.rowCount ?? 0),
          },
        }],
      },
    });
  ...
  const abc = pending.map((entry) => [
    dateInTimeZone(entry.movement.occurredAt, timeZone),
    entry.movement.product.sku,
    entry.movement.channel?.name ?? "初始化",
  ]);
  const ef = pending.map((entry) => [movementLabels[entry.movement.type], entry.movement.quantity]);
  const nr = pending.map((entry) => [
    entry.movementId,
    entry.movement.unitPrice == null ? "" : Number(entry.movement.unitPrice),
    entry.movement.referenceNo ?? "",
    entry.movement.note ?? "",
  ...
  await sheets.spreadsheets.batchUpdate({
    ...
    requestBody: {
      requests: [{
        copyPaste: {
          source: { sheetId: target.sheetId, startRowIndex: startRow - 1, endRowIndex: endRow, startColumnIndex: 12, endColumnIndex: 13 },
          destination: { sheetId: target.sheetId, startRowIndex: startRow - 1, endRowIndex: endRow, startColumnIndex: 13, endColumnIndex: 18 },
          pasteType: "PASTE_FORMAT",
          pasteOrientation: "NORMAL",
        },
      }],
  ...
  const verification = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range: `'庫存異動'!N${startRow}:N${endRow}`,
    valueRenderOption: "UNFORMATTED_VALUE",
  });
...
export async function processGoogleSheetMovementQueue(limit = 100) {
  ...
  await prisma.googleSheetMovementQueue.updateMany({
    where: { status: "PROCESSING", updatedAt: { lt: new Date(Date.now() - 10 * 60_000) } },
    data: {
      status: "FAILED",
      processingToken: null,
      lastError: "上次同步程序中斷，已重新排入等待區",
      nextAttemptAt: new Date(),
    },
  });
  const pendingCount = await prisma.googleSheetMovementQueue.count({
  ...
  if (!config.hasCredentials) {
    return {
      ...
      demo: true,
      message: "目前是本地 Demo；Queue 已保留，設定可寫入的 Service Account 後才會送出",
    };
  }

  const processingToken = randomUUID();
  const candidates = await prisma.googleSheetMovementQueue.findMany({
    where: {
  ...
  const claimed = await prisma.googleSheetMovementQueue.findMany({
  ...
  try {
    ...
    const syncedAt = new Date();
    await prisma.$transaction(claimed.map((item) => prisma.googleSheetMovementQueue.update({
      where: { id: item.id },
      data: {
        status: "SYNCED",
        ...
        sheetRow: sheetRows.get(item.movementId) ?? null,
        syncedAt,
        lastError: null,
      },
    })));
    return {
      ...
      failed: 0,
      pending: Math.max(0, pendingCount - claimed.length),
      demo: false,
    };
  } catch (error) {
...
export async function getGoogleSheetMovementQueueSummary() {
  const [counts, recent] = await Promise.all([
    ...
    prisma.googleSheetMovementQueue.findMany({
      include: {
        movement: {
          include: {
