...
type AutofillResult = {
  ...
  suggestedShippingGroupCount: number;
  suggestedReimbursableShippingFee: number;
  suggestedReimbursableShippingGroupCount: number;
  policy: {
    settlementCycle: SettlementCycle | null;
    billingTrigger: BillingTrigger | null;
    billingWithinDays: number | null;
...
function policySummary(channel: Channel) {
  const cycle = channel.settlementCycle ? cycleLabels[channel.settlementCycle] : "結算方式未設定";
  const trigger = channel.billingTrigger ? triggerLabels[channel.billingTrigger] : "觸發未設定";
  const deadline = channel.billingWithinDays == null ? null : `${channel.billingWithinDays} 天內請款`;
  return [cycle, trigger, deadline, channel.includeShippingInBilling ? "運費列入請款" : null, channel.requiresSalesInvoice ? "需開發票" : null].filter(Boolean).join(" · ");
}
...
export function BillingManager({
  ...
  const [shippingSourceCount, setShippingSourceCount] = useState(0);
  const [reimbursableShippingSourceCount, setReimbursableShippingSourceCount] = useState(0);
  const [reimbursableShippingFee, setReimbursableShippingFee] = useState(0);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const shippingGroups = result.suggestedShippingGroupCount ?? 0;
        const reimbursableGroups = result.suggestedReimbursableShippingGroupCount ?? 0;
        const reimbursableFee = result.suggestedReimbursableShippingFee ?? 0;
        const shouldAutofillShipping = Boolean(result.policy?.includeShippingInBilling) || reimbursableGroups > 0;
        setShippingSourceCount(shippingGroups);
        setReimbursableShippingSourceCount(reimbursableGroups);
        setReimbursableShippingFee(reimbursableFee);
        setShippingFee(shouldAutofillShipping ? String(result.suggestedShippingFee ?? 0) : "0");
        const shippingText = shouldAutofillShipping
          ? `；另帶入 ${shippingGroups} 批可請款運費 ${money(result.suggestedShippingFee)}${reimbursableGroups > 0 ? `，其中 ${reimbursableGroups} 批為公司代墊 ${money(reimbursableFee)}` : ""}`
          : "";
        setAutofillMessage(result.items.length > 0
          ? `找到 ${result.sourceMovementCount} 筆尚未結算銷貨，已彙整成 ${result.items.length} 個 SKU${shippingText}。建立後會鎖定銷貨來源並同步一筆應收到收支。`
          : `這個期間沒有尚未結算的銷貨${shippingText}；若是例外請款，仍可手動新增品項。`);
      } catch (error) {
  ...
  function selectChannel(nextId: string) {
    ...
    setShippingFee("0");
    setShippingSourceCount(0);
    setReimbursableShippingSourceCount(0);
    setReimbursableShippingFee(0);
  ...
  return <div className="billing-page">
    <PageHeader eyebrow="Receivables" title="請款 / 結算管理" description="選通路與期間後，自動彙整尚未結算的銷貨；通路主檔會帶入結算方式、請款觸發、運費與發票政策。" />
    ...
    {canWrite && <section className="billing-create-grid">
      <div className="panel billing-form-panel">
        ...
        {channel && <div className="billing-customer-card">
          ...
          <dl>
            ...
            <div className="wide"><dt>結算規則</dt><dd>{policySummary(channel)}</dd></div>
          </dl>
        </div>}
        {channel?.settlementCycle === "PER_SHIPMENT" && <p className="billing-hint">此通路設定為「每次出貨結算」；目前請以單次出貨的日期範圍建立，下一階段會再用到貨日直接形成待請款工作。</p>}
        {channel?.billingTrigger === "EXTERNAL_STATEMENT" && <p className="billing-hint">此通路設定為收到月結表後請款：先用通路表單核對這裡的待結算銷貨，再按確認建立結算。</p>}
        <div className="billing-two-col">
          ...
          <div className="field"><label>請款運費</label><input className="input" type="number" min="0" step="1" value={shippingFee} onChange={(event) => setShippingFee(event.target.value)} /><small className="helper">{reimbursableShippingSourceCount > 0
            ? `已帶入 ${reimbursableShippingSourceCount} 批公司代墊運費 ${money(reimbursableShippingFee)}；本期共 ${shippingSourceCount} 批可請款運費，可人工校正。`
            : channel?.includeShippingInBilling ? `依期間自動帶入 ${shippingSourceCount} 批通路 / 客戶負擔運費，可人工校正。` : "通路主檔未設定一般運費列入請款。"}</small></div>
        </div>
        ...
        {reimbursableShippingSourceCount > 0 && <p className="billing-hint">本期包含公司先支付、之後向通路回收的代墊運費。這些金額已包含在請款運費中，不需要再另外手動加一次。</p>}
        {channel?.requiresSalesInvoice && <p className="billing-hint">此通路主檔設定「需開銷項發票」；建立後 Finance 會標記為待處理發票。</p>}
      ...
      <div className="panel billing-preview-panel">
        ...
        {autofillLoading && previewItems.length === 0 ? <div className="billing-empty"><strong>正在整理待結算銷貨…</strong><span>只會抓尚未被其他結算使用的庫存異動。</span></div> : previewItems.length === 0 ? <div className="billing-empty"><strong>目前沒有待結算品項</strong><span>若是沒有對應庫存銷貨的例外請款，可以手動新增商品與數量。</span></div> : <>
          <div className="billing-preview-meta"><span>{previewItems.length} 個 SKU</span>{autoSettlement ? <span>{autofillSourceCount} 筆待結算銷貨</span> : <span>手動請款</span>}<span>{autoSettlement ? "建立後鎖定來源" : "無庫存來源連結"}</span>{reimbursableShippingSourceCount > 0 && <span>{reimbursableShippingSourceCount} 批代墊運費</span>}{channel?.requiresSalesInvoice && <span>需開發票</span>}</div>
          ...
          <div className="billing-total-box">
            <div><span>未稅金額</span><strong>{money(subtotal)}</strong></div><div><span>營業稅</span><strong>{money(taxAmount)}</strong></div><div><span>運費{reimbursableShippingSourceCount > 0 ? "（含代墊回收）" : channel?.includeShippingInBilling ? "（列入請款）" : ""}</span><strong>{money(Number.isFinite(shipping) ? shipping : 0)}</strong></div>
