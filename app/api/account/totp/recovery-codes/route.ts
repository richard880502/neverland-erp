...
import { decryptTotpSecret, generateRecoveryCodes, hashRecoveryCode, looksLikeRecoveryCode, verifyTotpCode } from "@/lib/two-factor";
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      if (existingRecovery) {
        ...
        const consumed = await tx.user.updateMany({ where: { id: user.id, OR: [{ totpLastUsedStep: null }, { totpLastUsedStep: { lt: timeStep } }] }, data: { totpLastUsedStep: timeStep } });
        if (consumed.count !== 1) throw new Error("TOTP_ALREADY_USED");
      }
      ...
      await tx.auditLog.create({ data: { userId: user.id, action: "TOTP_RECOVERY_CODES_REGENERATED", entityType: "User", entityId: user.id, metadata: { recoveryCodeCount: recoveryCodes.length }, ipAddress: clientIp(request) } });
    });
    ...
    return NextResponse.json({ error: "無法重新產生備援碼" }, { status: 500 });
  }
}
