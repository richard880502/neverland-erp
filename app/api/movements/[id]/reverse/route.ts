...
import { reverseInventoryMovement } from "@/lib/services/movements";

export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    assertSameOrigin(request); const auth = await requireApiUser({ roles: ["ADMIN", "STAFF"] }); const { id } = await context.params;
    await reverseInventoryMovement(id, { userId: auth.user.id, role: auth.user.role, ipAddress: clientIp(request) });
