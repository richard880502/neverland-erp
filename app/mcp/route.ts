...
import { authenticateMcpAccessToken, baseUrl, requireHttpsInProduction } from "@/lib/mcp/oauth";
...
function unauthorized(request: Request) { const metadata = `${baseUrl(request)}/.well-known/oauth-protected-resource`; return NextResponse.json({ error: "invalid_token" }, { status: 401, headers: { "WWW-Authenticate": `Bearer resource_metadata="${metadata}"`, "Cache-Control": "no-store" } }); }

export async function POST(request: Request) {
  try {
    requireHttpsInProduction(request);
    const ip = request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || request.headers.get("x-real-ip") || "unknown";
    const rate = takeRateLimit(`mcp:${ip}`); if (!rate.allowed) return NextResponse.json({ error: "rate_limited" }, { status: 429, headers: { "Retry-After": String(rate.retryAfter) } });
    const header = request.headers.get("authorization"); if (!header?.startsWith("Bearer ")) return unauthorized(request);
    const auth = await authenticateMcpAccessToken(header.slice(7), `${baseUrl(request)}/mcp`); if (!auth) return unauthorized(request);
    const body: unknown = await request.json(); if (!body || typeof body !== "object" || Array.isArray(body)) return error(null, -32600, "Invalid Request", 400);
    ...
    if (rpc.method === "tools/call") {
      ...
      try { const result = await callMcpTool(name, rpc.params?.arguments, auth); return isNotification ? new NextResponse(null, { status: 202 }) : response(rpc.id, result); }
      catch (cause) { const message = cause instanceof Error ? cause.message : "工具執行失敗"; return isNotification ? new NextResponse(null, { status: 202 }) : response(rpc.id, { content: [{ type: "text", text: message }], isError: true }); }
    }
    return error(rpc.id ?? null, -32601, "Method not found");
  } catch (cause) { return error(null, -32603, cause instanceof Error && cause.message === "MCP 必須透過 HTTPS 使用" ? cause.message : "Internal error", 400); }
}
