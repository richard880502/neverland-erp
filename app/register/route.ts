...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await registerDynamicClient({ redirectUris: input.data.redirect_uris, clientName: input.data.client_name, applicationType: input.data.application_type, grantTypes: input.data.grant_types, responseTypes: input.data.response_types, tokenEndpointAuthMethod: input.data.token_endpoint_auth_method }), { status: 201, headers: { "Cache-Control": "no-store" } });
